...
// input type="file" 태그의 값이 변경되었을 때
// 변경된 상태를 백업해서 저장할 변수
...
if (profile != null) {
  ...
  const changeImageFn = e => {
    ...
    if (file.size > maxSize) {
      ...
      if (status == -1) {
        imageInput.value = '';
      } else { // 기존 선택한 이미지가 있는데, 다음 선택한 이미지가 최대 크기를 초과한 경우

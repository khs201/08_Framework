...
@Controller // 요청/응답 제어 역할임을 명시하면서 Bean으로 등록
@RequestMapping("example") // /example로 시작하는 주소를 모두 해당 컨트롤러에 매핑
@Slf4j // lombok 라이브러리가 제공하는 로그 객체 자동 생성 어노테이션
public class ExampleController {

	@GetMapping("ex1") // /example/ex1 의 GET 방식 요청을 매핑하겠다는 의미
	public String ex1(HttpServletRequest req, Model model) {

		/*
		 * Model - Spring에서 데이터 전달 역할을 하는 객체 - org.springframework.ui 패키지 - 기본 scope :
		 * request - @SessionAttributes 와 함께 사용 시 session scope 변환
		 * 
		 * [기본 사용법] Model.addAttribute("key", value);
		 * 
		 * 
		 * (참고) (Model과 비슷한 ModelAndView도 존재) - ModelAndView : 데이터 전달 + forward할 파일 경로
		 * 지정
		 */
		...
		stdList.add(new Student("11111", "김일번", 10));
		stdList.add(new Student("22222", "김이번", 20));
		stdList.add(new Student("33333", "김삼번", 30));

		model.addAttribute("stdList", stdList);

		// forward == 요청 위임
		return "example/ex1"; // templates/example/ex1.html 경로의 html로 포워드한다는 의미
	}
	...
	public String ex4(Model model) {
		...
		Student std = new Student("67890", "잠만보", 39);
